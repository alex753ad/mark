...
├── train_ml.py                   # Переобучение ML-моделей на данных history.db
│
├── data/
...
| # | Признак | Ключ в `lvl` | Описание |
|---|---------|-------------|----------|
| 1 | `strength` | `"strength"` | Python-сила уровня (1-5) |
| 2 | `ltype_enc` | `"type"` | Тип уровня (закодированный через `level_type_map`) |
| 3 | `vol` | `"vol_ratio"` | Объём при подходе (обрезается до 20) |
| 4 | `touches` | `"approach"` | Число касаний / подходов к уровню (**обрезается до 5**) |
| 5 | `atr_ratio` | `"atr_ratio"` | Расстояние до уровня в ATR (обрезается до 20) |
| 6 | `style_enc` | `"approach_style"` | **Стиль подхода** (см. ниже), закодированный через `style_map` |

**`approach_style` — энкодинг и семантика:**

```python
style_map = {"flash": 0, "impulse": 1, "bleed": 2, "unknown": 3}
```

| Стиль | Значение | Типичное поведение | Ожидание модели |
|-------|----------|--------------------|-----------------|
| `flash` | 0 | Одна резкая свеча ×2 объём, движение ≥ 0.5% | Частый sweep перед разворотом → **bounce вероятнее** |
| `impulse` | 1 | 3+ зелёных свечей подряд, затем красная | Первое серьёзное тестирование уровня → **нейтрально** |
| `bleed` | 2 | 4+ красных свечи с растущим объёмом | Методичное давление продавцов → **bounce менее вероятен** |
| `unknown` | 3 | Паттерн не определён | Fallback при отсутствии данных |

**⚠️ Важно — момент вычисления:**
`approach_style` определяется функцией `detect_approach_style()` **в момент мониторинга** (при касании уровня), а не при построении уровней. Поэтому:

- При автотриггере (`_run_phase1`) и стартовом мониторинге значение **недоступно** → передаётся `"unknown"`
- При вызове из монитора (`_monitored`) → значение из `detect_approach_style()` передаётся явно через `lvl["approach_style"]`
- `apply_ml_to_level(lvl)` читает `lvl.get("approach_style", "unknown")` — признак должен быть выставлен **до вызова функции**
...
features = [
    lvl.get("strength", 3),
    ltype_enc,                                          # из level_type_map
    min(lvl.get("vol_ratio", 1.0), 20.0),
    min(float(lvl.get("approach", 0) or 0), 5.0),   # обрезается до 5
    min(lvl.get("atr_ratio", 2.0), 20.0),
    style_map.get(lvl.get("approach_style", "unknown"), 3),  # признак 6
]
```
...
| Функция | Описание |
|---------|----------|
| `ml_score(lvl: dict) -> dict` | Считает `{p_bounce, expected_depth, ml_delta}`. Использует 6 признаков включая `approach_style`. При недоступных моделях возвращает нейтраль `{p_bounce=0.5, expected_depth=1.5, ml_delta=0}` |
| `apply_ml_to_level(lvl: dict) -> None` | Вызывается после `calculate_strength(lvl)`. Читает `lvl["approach_style"]` (если не задан — `"unknown"`). Применяет `ml_delta` к `strength`, добавляет `p_bounce`, `expected_depth`, `strength_pre_ml`, `ml_delta` в `lvl`. **CAP:** если `was_broken && !sweep_reclaimed` — ML не корректирует вверх (только 0 или -1) |
| `_load() -> bool` | Lazy-загрузка pickle-файлов. Повторные вызовы — no-op. |

**`ml_delta` логика:**

```python
# Пороги откалиброваны по реальным квартилям bounce-rate
# (верхний квартиль ~0.57, нижний ~0.32 на датасете 2026-05-21+, n=777)
if p_bounce >= 0.57:   ml_delta = +1
elif p_bounce <= 0.32: ml_delta = -1
else:                  ml_delta =  0

strength = max(1, min(5, strength + ml_delta))
```

**Пример вызова (в `_monitored`, после определения стиля):**

```python
approach_style = detect_approach_style(symbol)
lvl["approach_style"] = approach_style   # выставить ДО вызова ML

calculate_strength(lvl)
apply_ml_to_level(lvl)  # читает lvl["approach_style"], мутирует lvl на месте
```

**Пример вызова без стиля (в `_run_phase1`, стиль неизвестен):**

```python
# approach_style недоступен на этапе построения уровней
# apply_ml_to_level использует fallback "unknown" → style_enc = 3
calculate_strength(lvl)
...
5. `apply_ml_to_level()` — ML-поправка к strength, добавляет `p_bounce`, `ml_delta`
6. Claude `calculate_strength_with_claude()` (если `CLAUDE_STRENGTH_ENABLED`)
7. **Cap:** Claude не может дать выше Python, если approach ≥ 2 или was_broken && !sweep
8. Разделение: strong (≥ 4) и weak (< 4)
9. Telegram: текст с уровнями + звёздами + claude_reason + `🤖 P(отбой): X% | ML: ±1` (если ml_delta ≠ 0)
...
6. Telegram: полный отчёт + статистика исходов + сетка + уверенность + `🤖 P(отбой): X% | ML: ±1` (если ml_delta ≠ 0)
...
Добавляет в уровень: `p_bounce`, `expected_depth`, `strength_pre_ml`, `ml_delta`, корректирует `strength` на ±1. CAP: `was_broken && !sweep_reclaimed` → ML не повышает.
**Использует 6 признаков**, включая `approach_style` — стиль подхода к уровню (flash / impulse / bleed / unknown). Признак должен быть записан в `lvl["approach_style"]` до вызова; при отсутствии — автоматически подставляется `"unknown"`.
...
Python calculate_strength()
    ...
    python_strength (сохраняется)
          │
          │   detect_approach_style()     ← только в _monitored()
          │          │                       при _run_phase1 = "unknown"
          │          ▼
          │   lvl["approach_style"] = style
          │
          ▼
apply_ml_to_level()   ← 6 признаков: strength, type, vol, touches(≤5), atr_ratio(≤20), approach_style
          │            → p_bounce, expected_depth, ml_delta (±1)
          ▼
    strength после ML
          │
...
| `train_ml.py` | Скрипт переобучения ML-моделей: фильтрация `history.db`, обучение `clf`/`reg`, перезапись `.pkl` |
