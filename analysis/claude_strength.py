...
ШТРАФЫ ЗА КАСАНИЯ (обязательно):
- touches = 1 → без ограничений
- touches = 2 → максимум strength = 3
- touches 3–4 → максимум strength = 2
- touches ≥ 5 → максимум strength = 1
- pump_base + touches > 1 → не может быть strength = 5

УСЛОВИЯ ДЛЯ strength=5 (все три должны выполняться):
- touches = 1 (только первое касание)
- vol_ratio_on_approach >= 1.5 (объём выше среднего)
- тип уровня не pump_base (или pump_base только при touches = 1 и высоком объёме)
Если хотя бы одно условие не выполнено — максимум strength = 4
