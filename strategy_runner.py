...
async def run_strategies() -> None:
    ...
    asyncio.create_task(_price_loop(strategies, s2_live))
...
async def _price_loop(strategies: list, s2_live) -> None:
    """
    ...
    while True:
        ...
        try:
            ...
            for symbol in symbols:
                ...
                try:
                    await s2_live.check_exits(symbol, current_price)
                except Exception as e:
                    logger.error("price_loop s2_live.check_exits error", symbol=symbol, error=str(e))
        except Exception as e:
            logger.error("price_loop error", error=str(e))


async def _timeout_checker(strategies: list, s2_live) -> None:
    ...
    while True:
        ...
        try:
            await s2_live.check_timeout()
        except Exception as e:
            logger.error("s2_live check_timeout error", error=str(e))
